...
def render_for_prompt(snap: dict, limit: int = 45) -> str:
    ...
    lines.append("")
    lines.append(f"조작 가능한 요소 ({len(snap['elements'])}개, 상위 {limit}개 표시):")

    # 접힘선 위 → 아래 순, 각 구간은 위에서부터
    els = sorted(snap["elements"], key=lambda e: (e["below_fold"], e["y"], e["x"]))
    for el in els[:limit]:
        ...
        lines.append(
            f"  {el['id']}: \"{text}\"{extra}" + (f"  ⟨{flags}⟩" if flags else "")
        )
    if len(els) > limit:
        lines.append(f"  … 외 {len(els) - limit}개")
    return "\n".join(lines)
