 ...
 *
 * 찍지 못하는 주소(죽은 사이트, 로그인 벽)는 Figma 의 기본 이미지로 떨어진다.
 *
 * fit:
...
export function SitePreview({
  ...
  if (!url || failed) {
    return <img src={placeholder} alt={alt} className="size-full object-cover" />
  }
  ...
  return (
    <img
      ...
      className={`size-full object-cover ${fit === 'cover' ? 'object-center' : 'object-top'}`}
    />
  )
}
